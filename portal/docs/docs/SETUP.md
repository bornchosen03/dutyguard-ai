...
- Frontend: Vite + React. Production build is in `customer-portal/dist/`.
...
- Frontend dev server: in `customer-portal/` run `npm run dev`
...
- `.gitignore` added to exclude `node_modules/`, `customer-portal/dist/`, `backend/.venv*`, and `logs/`.
- Tracked `customer-portal/node_modules` was removed from the index.
