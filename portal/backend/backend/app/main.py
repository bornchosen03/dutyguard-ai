...
Serves the built customer portal from `customer-portal/dist` and provides API endpoints:
...
UPLOAD_RATE_LIMIT_MAX = int(os.getenv("DUTYGUARD_UPLOAD_RATE_MAX", "20"))
UPLOAD_RATE_WINDOW = int(os.getenv("DUTYGUARD_UPLOAD_RATE_WINDOW", "3600"))
...
async def security_headers_middleware(request, call_next):
    ...
    # Content Security Policy: allow same-origin and trusted scripts (adjust as needed)
    response.headers.setdefault(
        "Content-Security-Policy",
        "default-src 'self'; script-src 'self' 'unsafe-inline' https:; style-src 'self' 'unsafe-inline' https:; img-src 'self' data: https:;"
    )
    # HSTS (safe to enable when using HTTPS in production)
...
async def https_redirect_middleware(request, call_next):
    """Optional HTTPS redirect when `DUTYGUARD_FORCE_HTTPS` is set to true.

    In many deployments HTTPS is handled at the load-balancer and this redirect
    is not necessary; enable only when appropriate for your environment.
    """
    if _bool_env("DUTYGUARD_FORCE_HTTPS", False):
...
async def upload_tariff_file(request: Request, file: UploadFile = File(...)) -> dict[str, Any]:
    ...
    # Rate-limit uploads per user to avoid abuse
    now_ts = time.time()
    if _rate_limit_exceeded(f"upload_user:{user_key}", now_ts, max_requests=UPLOAD_RATE_LIMIT_MAX, window_seconds=UPLOAD_RATE_WINDOW):
        raise HTTPException(status_code=429, detail="Too many uploads, please try again later")
    # Also rate-limit by client IP
    client_ip = (request.client.host if request.client and request.client.host else "unknown").strip().lower()
    if _rate_limit_exceeded(f"upload_ip:{client_ip}", now_ts, max_requests=UPLOAD_RATE_LIMIT_MAX * 2, window_seconds=UPLOAD_RATE_WINDOW):
    ...
    if files:
        ...
        for f in files:
            ...
            if not content:
                continue
            if len(content) > MAX_UPLOAD_BYTES:
                raise HTTPException(status_code=413, detail=f"Uploaded file '{filename}' exceeds allowed size")
            total_bytes += len(content)
            if total_bytes > MAX_TOTAL_INTAKE_BYTES:
                raise HTTPException(status_code=413, detail="Total uploaded files exceed allowed intake size")
            sha256 = hashlib.sha256(content).hexdigest()
...
def spa_fallback(full_path: str):
    if not DIST_DIR.exists():
        raise HTTPException(status_code=404, detail="Customer portal not built")
