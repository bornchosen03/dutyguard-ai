...
# Build customer portal and start backend (serves static assets)
...
# Fast mode defaults:
# - skip customer portal npm install unless explicitly forced
# - skip customer portal build if dist already exists unless explicitly forced
SKIP_FRONTEND_INSTALL="${SKIP_FRONTEND_INSTALL:-1}"
FORCE_FRONTEND_BUILD="${FORCE_FRONTEND_BUILD:-0}"

echo "[start] Installing customer portal dependencies (safe, uses legacy-peer-deps)"
...
if command -v npm >/dev/null 2>&1; then
	if [ "$SKIP_FRONTEND_INSTALL" = "0" ]; then
		...
		echo "[start] Skipping npm install (set SKIP_FRONTEND_INSTALL=0 to force)"
	       ...
	       if [ "$FORCE_FRONTEND_BUILD" = "1" ] || [ ! -f "$ROOT/customer-portal/dist/index.html" ]; then
		       echo "[start] Building customer portal (production)"
		       npm run build
	       else
		       echo "[start] Skipping customer portal build (dist exists; set FORCE_FRONTEND_BUILD=1 to force)"
	       fi
else
	echo "[start] npm not found in PATH; skipping customer portal install/build"
fi
